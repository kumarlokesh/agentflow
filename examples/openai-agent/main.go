// Command openai-agent demonstrates an end-to-end agentflow run using the
// OpenAI Chat Completions API with the calculator tool, full observability
// hooks, and deterministic replay.
//
// Usage:
//
//	export OPENAI_API_KEY=sk-...
//	go run ./examples/openai-agent
//	go run ./examples/openai-agent -replay <run-id>
package main
...
func runAgent(ctx context.Context, fileStore agentflow.EventStore, apiKey, task string, logger *slog.Logger) {
	// Wire the OpenAI provider with retry.
	openaiClient := openai.New(apiKey,
		openai.WithModel("gpt-4o-mini"),
	)
	llm := retry.Wrap(openaiClient,
	...
	fmt.Printf("\nTo replay this run:\n")
	fmt.Printf("  go run ./examples/openai-agent -replay %s\n", result.RunID)
